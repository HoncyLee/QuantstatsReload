...
def snapshot(
    ...
    **kwargs,
    ...
    fig.suptitle(
        title, fontsize=14, y=0.97, #fontname=fontname, 
    ...
    axes[0].set_ylabel(
        "Cumulative Return", #fontname=fontname, 
    ...
    axes[2].set_ylabel(
        "Daily Return", #fontname=fontname, 
        fontweight="bold", fontsize=12
    )
    if isinstance(returns, _pd.Series):
        axes[2].plot(
...
def earnings(
    ...
    fig.suptitle(
        f"    {title}", fontsize=12, y=0.95, #fontname=fontname, 
        fontweight="bold", color="black"
    )
